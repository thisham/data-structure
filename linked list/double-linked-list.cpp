 ...
 * 
 * Riwayat Perubahan :
 * 		- Perombakan dari SLL menjadi DLL.
 * 
 */
...
void addNode(node **head, node *previous, node *atarashii) {
	if (previous == NULL) {
		...
		atarashii->next = *head;
		atarashii->previous = NULL;
		*head = atarashii;
	} else {
		// Add new chain at the end and at middle.
		atarashii->next = previous->next;
		previous->next = atarashii;
		atarashii->previous = previous;
	}
}
...
void addEnd(node **head) {
	...
	newPointer->previous = currentPointer;
	return;
}
